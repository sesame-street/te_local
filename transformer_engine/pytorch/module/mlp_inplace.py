...
class _EmptyMLPIn(torch.autograd.Function):
    ...
    def backward(
        ...
        with _prepare_backward(
            ctx.fp8, ctx.fp8_meta, ctx.tp_group, ctx.tp_size, name="_MLPIn"
        ):
            pass
...
class _MLPIn(torch.autograd.Function):
    """MLP semi-top level module
    Calls custom cuda extensions.
    """
    ...
    def forward(
        ...
        assert not ub_overlap_ag, 'not tested'
        ...
        if fp8:
            ...
            ln_out = cast_if_needed(inputmat, activation_dtype)
        ...
        if ub_overlap_ag:
            ...
            ln_out_total = ln_out
        assert not (set_parallel_mode and sequence_parallel), 'not tested yet'
        ...
        if fp8:
            ...
            assert not (ub_split_rs or ub_atomic_gemm_rs), "not implemented"
            # if ub_split_rs or ub_atomic_gemm_rs:
        ...
        assert not (ub_split_rs or ub_atomic_gemm_rs), "not implemented!"
        assert not (set_parallel_mode and sequence_parallel), "not implemented!"
        assert not (set_parallel_mode and tensor_parallel), "not implemented!"
    ...
    def backward(
        ...
        with _prepare_backward(
            ...
            # LayerNorm gradient
            dgrad = fc1_dgrad.view(inputmat_shape)
...
class MLPIn(TransformerEngineBaseModule):
    r"""
        ...
        with self.prepare_forward(inp, is_first_microbatch, num_gemms=2) as inp:
            ...
            if inp.nelement() == 0:
                if torch.is_grad_enabled():
                    fwd_fn = _EmptyMLPIn.apply
                    args = []
                else:
                    fwd_fn = _EmptyMLPIn.forward
                    args = [None]
                args += (
                    inp,
                    out,

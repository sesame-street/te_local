...
class _EmptyMLP(torch.autograd.Function):
    ...
    def backward(
        ...
        with _prepare_backward(
            ctx.fp8, ctx.fp8_meta, ctx.tp_group, ctx.tp_size, name="_MLP"
        ):
            pass
...
class MLP(TransformerEngineBaseModule):
    r"""
        ...
        with self.prepare_forward(inp, is_first_microbatch, num_gemms=2) as inp:
            ...
            if inp.nelement() == 0:
                if torch.is_grad_enabled():
                    fwd_fn = _EmptyMLP.apply
                    args = []
                else:
                    fwd_fn = _EmptyMLP.forward
                    args = [None]
                args += (
                    inp,
                    self.fc1_weight,
